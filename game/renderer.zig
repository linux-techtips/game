...
pub const Context = struct {
    ...
    pub fn init(window: *Engine.Window) Context {
    ...
    pub fn deinit(ctx: *const Context) void {
        ...
        ctx.depth_view.release();
        ctx.queue.release();
        ctx.device.release();
        ctx.surface.release();
    }

    pub fn reconfigure(ctx: *Context, width: u32, height: u32) void {
        ctx.depth_view.release();

        ctx.depth_texture.destroy();
        ctx.depth_texture.release();

        ctx.depth_texture, ctx.depth_view = createDepthTextureAndView(ctx.device, width, height);

        configureSurface(ctx.device, ctx.surface, width, height);
    }
...
pub const Frame = struct {
    ...
    pub fn begin(ctx: *Context) Frame {
        const surface_texture = blk: {
            var res: gpu.SurfaceTexture = undefined;
            ctx.surface.getCurrentTexture(&res);
    ...
    pub fn end(frame: *const Frame, ctx: *Context) void {
